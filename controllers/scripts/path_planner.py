...
def simplificar_ruta(ruta):
    """
    Toma una ruta densa y elimina los nodos intermedios en línea recta.
    Solo conserva el nodo inicial, los puntos de giro (esquinas) y la meta.
    """
    if len(ruta) < 3:
        return ruta
        
    ruta_simplificada = [ruta[0]]
    ...
    for i in range(1, len(ruta) - 1):
        ...
        # Calcula el vector de dirección entre los nodos
        dir_x1 = nodo_act[0] - nodo_ant[0]
        dir_y1 = nodo_act[1] - nodo_ant[1]
        
        dir_x2 = nodo_sig[0] - nodo_act[0]
        dir_y2 = nodo_sig[1] - nodo_act[1]
        
        # Si la dirección cambia, es una esquina y la guardamos
        if dir_x1 != dir_x2 or dir_y1 != dir_y2:
            ruta_simplificada.append(nodo_act)
            
    ruta_simplificada.append(ruta[-1])
    return ruta_simplificada
